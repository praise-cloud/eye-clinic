...
class DatabaseService {
    ...
    async getMessages(userId, otherUserId = null, search = '', limit = 100, offset = 0) {
        const db = await this.getDatabase();
        let query = `
            SELECT c.*,
                   (s.first_name || ' ' || s.last_name) as sender_name, s.role as sender_role,
                   (r.first_name || ' ' || r.last_name) as receiver_name, r.role as receiver_role
            FROM chat c
    ...
    async sendMessage(senderId, receiverId, messageText, attachment = null, replyToId = null) {
        ...
        const query = `
            ...
            VALUES (?, ?, ?, ?, ?)
        `;
        const result = await db.run(query, [senderId, receiverId, messageText, attachment, replyToId]);
    ...
    async logActivity(userId, actionType, entityType, entityId, description, ipAddress = null, userAgent = null) {
        ...
        try {
            await db.run(query, [uuidv4(), userId, actionType, entityType, entityId, description, ipAddress, userAgent]);
        } catch (error) {
    ...
    async getActivityLogs(filters = {}) {
        const db = await this.getDatabase();
        let query = `
            SELECT a.*, (u.first_name || ' ' || u.last_name) as user_name, u.role as user_role
    ...
    async getActivityStatistics() {
        ...
        const [totalActivities, todayActivities, userActivity, entityActivity] = await Promise.all([
            ...
            db.all('SELECT (u.first_name || " " || u.last_name) as name, u.role, COUNT(*) as activity_count FROM activity_logs a LEFT JOIN users u ON a.user_id = u.id GROUP BY a.user_id ORDER BY activity_count DESC LIMIT 5'),
    ...
    async updateUserStatus(userId, isActive, updatedBy) {
        ...
        return await db.get('SELECT id, first_name, last_name, email, role, status FROM users WHERE id = ?', [userId]);
    }
    ...
    async updateUser(userId, userData, updatedBy) {
        ...
        if (first_name !== undefined) { fieldsToUpdate.push('first_name = ?'); params.push(first_name); }
        if (last_name !== undefined) { fieldsToUpdate.push('last_name = ?'); params.push(last_name); }
        if (email !== undefined) { fieldsToUpdate.push('email = ?'); params.push(email); }
        if (role !== undefined) { fieldsToUpdate.push('role = ?'); params.push(role); }
        if (phone_number !== undefined) { fieldsToUpdate.push('phone_number = ?'); params.push(phone_number); }
        if (gender !== undefined) { fieldsToUpdate.push('gender = ?'); params.push(gender); }
        if (password !== undefined && password !== '') {
            const saltRounds = 10;
            const passwordHash = await require('bcryptjs').hash(password, saltRounds);
    ...
    async deleteUser(userId, deletedBy) {
        ...
        return { success: result.changes > 0, deletedUser: `${user.first_name} ${user.last_name}` };
    }
